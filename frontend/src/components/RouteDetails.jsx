  ...
  useEffect(() => {
    const fetchRouteDetails = async () => {
      try {
        ...
        const routeData = {
          origin: {
            lat: activitiesWithGeometry[0].geometry.location.lat,
            lng: activitiesWithGeometry[0].geometry.location.lng
          },
          destination: {
            lat: activitiesWithGeometry[activitiesWithGeometry.length - 1].geometry.location.lat,
            lng: activitiesWithGeometry[activitiesWithGeometry.length - 1].geometry.location.lng
          },
          waypoints: activitiesWithGeometry.slice(1, -1).map(activity => ({
            lat: activity.geometry.location.lat,
            lng: activity.geometry.location.lng
          }))
        };

        // Make request to your backend instead of directly to Google Maps
        ...
        setDirections(directionsData.routes[0].legs);

        // Generate static map URL (this part remains the same)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gray-700 rounded-lg p-6 space-y-8">
          <div>
            ...
            {directions && directions.map((leg, legIndex) => (
              <div key={legIndex} className="mb-6">
                <h3 className="text-lg font-medium mb-2">
                  Stop {legIndex + 1}: {activities[legIndex].location} → {activities[legIndex + 1].location}
                </h3>
